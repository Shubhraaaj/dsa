...
public class RecursionBacktracking{
     ...
     * @param pos - Starting position will be 0 and will get incremented after adding new element
     * @param selected - Boolean array to find out whether the number was selected before or not
    ...
    // 3**. Optimised nQueens
    public static void nQueens2(char arr[][], int row, boolean[] columns, boolean leftDiagnol[], boolean[] rightDiagnol){
        if(row==arr.length){
            print2DArray(arr);
            System.out.println("----------");
            return;
        ...
        for(int col=0;col<arr[0].length;col++){
            if(columns[col]==false && leftDiagnol[row+col]==false && rightDiagnol[row-col+arr.length-1]==false){
                arr[row][col]='Q';
                columns[col]=true;
